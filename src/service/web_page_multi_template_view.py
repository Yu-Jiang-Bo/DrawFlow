...
MULTI_TEMPLATE_RENDER_VIEW_SCRIPT = r'''
    ...
    function renderMultiTemplateIssues(result) {
      ...
      const hasPartialOutput = Boolean((result.outputs || {}).partial_output_available);
      target.className = issues.length || hasPartialOutput ? "message error" : "message";
      ...
      if (hasPartialOutput && incompleteGroups.length) {
        ...
        missing.textContent = `部分包不包含以下尚未交付的模板订单：${details.join("；")}。`;
    ...
    function renderMultiTemplateActions(result) {
      ...
      document.getElementById("multiTemplatePartialDownloadBtn").hidden = !actions.can_download_partial;
    }
