...
historical configurations retain their legacy behavior across ordinary saves
until the enabled legacy mode migrates or an administrator selects a mode.
"""
...
def render_mode(config: Mapping[str, Any], render_task: Mapping[str, Any] | None = None) -> str | None:
    ...
    A missing field marks a historical configuration and retains its old
    execution behavior across ordinary saves.  Only the legacy enabled mode
    migrates to multi customization; otherwise an administrator must select a
    mode explicitly.  A compiled task must agree with that mode when present.
    """
    ...
    if isinstance(render_task, Mapping) and task_mode != configured:
        raise V2OrderRenderError(
...
def _quantity(
    ...
    *,
    ...
    if raw is None or not str(raw).strip():
        raise V2OrderRenderError(
            "当前模板启用了按购买数量出图，订单表需要提供数量列，且数量必须是正整数。",
