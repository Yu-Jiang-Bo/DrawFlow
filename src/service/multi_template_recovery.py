...
class MultiTemplateRecoveryCoordinator:
    ...
    def resume(self, parent_job_id: str) -> dict[str, Any]:
        ...
        if selected:
            ...
            return self._resume_selected_snapshot(record, selected)
        if not preflight_snapshot_is_intact(record):
            return self.invalidate_preflight(record)
        return self._dispatch_existing_ready(record, stage="resuming")

    def _resume_selected_snapshot(self, record: dict[str, Any], selected: tuple[str, ...]) -> dict[str, Any]:
        """Resume from the parent's immutable template and order snapshots."""
        ...
        snapshots = {
            ...
            for item in metadata.get("template_snapshots") or []
            if isinstance(item, Mapping)
        }
        groups = {
            str(item.get("template_id") or ""): item
            for item in preflight.get("groups") or []
            if isinstance(item, Mapping)
        }
        if any(template_id not in snapshots or template_id not in groups for template_id in selected):
            return self.invalidate_preflight(record)
        ...
        for checkpoint in current:
            ...
            if template_id in selected_set:
                reset_for_recovery(checkpoint, snapshots[template_id], groups[template_id])
        ...
        history.append({"mode": "resume", "template_ids": list(selected), "at": utc_now()})
        metadata.update({
            "template_checkpoints": current,
            "child_jobs": child_jobs,
            "recovery_history": history,
        })
        metadata.pop("interrupted_template_id", None)
        metadata.pop("recovery_preflight_error", None)
        record["multi_template"] = metadata
        resumed = self.jobs.update(
            record,
            status="ready",
            error="",
            error_code="",
            progress={
                "current": finished_checkpoint_count(current),
                "total": len(current),
                "stage": "resume_ready",
            },
        )
        return self._dispatch_existing_ready(resumed, stage="resuming")
