...
class TemplateRegistry:
    ...
    def apply_confirmed_rule_pack(
        ...
        *,
        ...
        output = _pack_output_settings(pack)
        if "outline_text" in output:
            target["outline_text"] = _to_bool(output["outline_text"])
        if "pathfinder_merge" in output:
            target["pathfinder_merge"] = _to_bool(output["pathfinder_merge"])
        if activate:
...
def _pack_output_settings(pack: Dict[str, Any]) -> Dict[str, Any]:
    rules = pack.get("rules") if isinstance(pack, dict) else None
    if isinstance(rules, dict) and isinstance(rules.get("output"), dict):
        return dict(rules["output"])
    if isinstance(pack.get("output"), dict):
        return dict(pack["output"])
    return {}
