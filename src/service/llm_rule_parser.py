            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "output, slots, assets, effects。"
                        "模板特殊规则只能编译到 effects 数组，禁止返回脚本、表达式或可执行代码。"
                        "每个 effect 使用 stage=text、target={field,name}、selector={type,delimiter,positions,trim}、actions。"
                        "selector.type 只能是 whole 或 split；positions 只能是 all、odd、even。"
                        "action.type 只能是 set_fill_color、set_font_size、set_tracking、set_horizontal_scale、"
                        "set_vertical_scale、set_baseline_shift。action.value 使用 literal/value 或 cycle/values。"
                        "例如名字按顺序红白交替应使用 split 选择器和 set_fill_color 的 cycle values，"
                        "不得创建 alternating_color、odd_color 等专用字段。"
