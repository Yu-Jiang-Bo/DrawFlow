...
def check_initial_with_text_values(
    ...
    if len(asset_slots) != 1 or not text_slots:
        issues.append(_content_issue(contract, output, option, row_index, order_id, path, "initial_preset_incomplete", "首字母素材加正文必须包含一个素材槽位和一个正文槽位。"))
        return
    ...
    try:
        ...
        match_supported_asset_value(parsed.initial, _asset_supported_values(option, str(asset_slot.get("asset_key") or "")))
...
def check_multi_initial_values(
    ...
    if len(asset_slots) < 2:
        issues.append(_content_issue(contract, output, option, row_index, order_id, path, "multi_initials_incomplete", "多首字母提取必须包含至少两个素材槽位。"))
        return
    bindings = dict(contract.get("field_bindings") or {})
    ...
    try:
        ...
        for slot, initial in zip(asset_slots, initials):
            match_supported_asset_value(initial, _asset_supported_values(option, str(slot.get("asset_key") or "")))
    except V2ContentPresetError as exc:
        issues.append(
            _content_issue(
                contract,
