...
def main() -> int:
    ...
    except (LauncherError, UpdateTransportError) as exc:
        _write_log(root, f"update skipped: {exc}")
    if candidate is not None:
        try:
            process = _start(runtime, candidate.version, config)
        except OSError as exc:
            runtime.discard_candidate(candidate.version)
...
def _start(runtime: LauncherRuntime, version: str, config: Mapping[str, Any]) -> subprocess.Popen[bytes]:
    environment = os.environ.copy()
    central_url = str(config.get("central_url") or "").strip()
    if central_url:
        environment["DRAWFLOW_CENTRAL_URL"] = central_url.rstrip("/")
    return subprocess.Popen([str(runtime.executable_path(version))], env=environment)
