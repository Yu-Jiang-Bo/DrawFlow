...
def test_page_uses_the_desktop_production_workbench_shell_without_changing_business_entries():
    ...
    assert 'id="renderTemplateContext"' in INDEX_HTML
    assert 'id="renderRecentJobs"' in INDEX_HTML
    assert "function renderRenderContext" in INDEX_HTML
...
def test_desktop_render_context_only_uses_loaded_template_and_local_jobs():
    function = embedded_page_function("renderRenderContext")
    run_page_behavior(
        """
        const assert = require("assert");
        const templateTarget = { innerHTML: "" };
        const jobsTarget = { innerHTML: "" };
        const document = { getElementById: (id) => ({ renderTemplateContext: templateTarget, renderRecentJobs: jobsTarget })[id] || null };
        const state = {
          jobs: [
            { job_id: "job-1", status: "completed", created_at: "2026-08-28T10:00:00Z", request: { template_id: "T-1" }, outputs: { output_ai: "out.ai" } },
            { job_id: "job-2", status: "running", created_at: "2026-08-28T10:01:00Z", request: { template_id: "T-2" }, outputs: {} }
          ]
        };
        function selectedTemplate() { return { template_id: "T-1", name: "真实模板", status: "active" }; }
        function escapeHtml(value) { return String(value); }
        function displayStatus(value) { return ({ active: "启用", completed: "完成", running: "运行中" })[value] || value; }
        function formatDate(value) { return value; }
        function primaryOutputKey(outputs) { return outputs.output_ai ? "output_ai" : ""; }
        function deliveryDownloadLabel() { return "下载 AI 成品"; }
        """
        + function
        + """
        renderRenderContext();
        assert(templateTarget.innerHTML.includes("真实模板"));
        assert(jobsTarget.innerHTML.includes("T-1"));
        assert(jobsTarget.innerHTML.includes("/api/jobs/job-1/download/output_ai"));
        assert(!jobsTarget.innerHTML.includes("模板示例"));
        """
    )
