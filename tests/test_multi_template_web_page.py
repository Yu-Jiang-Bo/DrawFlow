...
from src.service.web_page import workbench_html
...
def test_multi_template_browser_result_lists_incomplete_scope_without_partial_download():
    program = f'''
const vm = require("vm");
...
const elements = {{
  ...
  multiTemplatePrimaryDownloadBtn: node()
}};
...
renderMultiTemplateResult({{
  ...
  actions: {{ execute: false, can_retry_failed: true, can_resume: false }},
  outputs: {{ primary_output_available: false }}
}});
...
if (elements.multiTemplateRetryBtn.hidden) throw new Error("recoverable retry control was hidden");
...
if (!elements.multiTemplateIssueList.className || elements.multiTemplatePrimaryDownloadBtn.hidden !== false) throw new Error("complete delivery control was not enabled");
'''

    subprocess.run(["node", "-e", program], check=True, capture_output=True, text=True)


def test_multi_template_page_has_no_partial_download_control():
    assert "multiTemplatePartialDownloadBtn" not in workbench_html()
    assert "/output/partial" not in MULTI_TEMPLATE_RENDER_SCRIPT
