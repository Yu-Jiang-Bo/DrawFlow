...
@pytest.mark.parametrize(
    ...
    (("render_task", "render_202508_grouped.jsx"), ("render_task_batch", "render_202508_batch.jsx")),
)
def test_202508_direct_render_entrypoints_construct_and_call_bridge(tmp_path, monkeypatch, entrypoint, script_name):
    calls = []
    ...
    class Bridge:
        ...
        def render(self, script, task_file):
            calls.append(("render", Path(script).name, Path(task_file).name))

    monkeypatch.setattr(jjmb_202508_main, "IllustratorBridge", Bridge)

    getattr(jjmb_202508_main, entrypoint)(tmp_path / "task.json", False)

    assert calls == [("init", {"visible": False}), ("render", script_name, "task.json")]


@pytest.mark.parametrize("runner", ("standalone", "config_export", "production_batch"))
def test_auto_closing_renderers_never_attach_to_shared_dispatch(tmp_path, monkeypatch, runner):
    dispatch_calls = []
    client = types.ModuleType("win32com.client")
    client.Dispatch = lambda _prog_id: dispatch_calls.append("Dispatch")
    ...
    with pytest.raises(IllustratorBridgeError, match="缺少 DispatchEx"):
        if runner == "standalone":
            render_service._render_standalone_illustrator_task(False, script, task)
        elif runner == "config_export":
            export_template_config.render_template_config_task(False, script, task)
        else:
            bridge = production_batch._new_isolated_bridge(False)
            try:
                bridge.render(script, task)
            finally:
                bridge.close()
