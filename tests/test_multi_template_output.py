...
def test_collector_uses_partial_zip_for_only_successful_templates(tmp_path):
    ...
    result = MultiTemplateResultCollector().collect(parent, status="completed_with_errors")

    assert result.kind == "partial_output"
    assert "部分成功" in result.path.name
    with zipfile.ZipFile(result.path) as archive:
        assert archive.namelist() == ["templates/A/department/K/ORDER1.ai"]
