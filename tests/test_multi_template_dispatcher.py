...
class PublicationStatePreflight(BatchPreflight):
    ...
    def preflight(self, order_file, *, sheet_name, work_dir):
        self.calls += 1
        if not self.active:
            raise MultiTemplateRenderError("模板已停用。", code="template_inactive")
        result = super().preflight(order_file, sheet_name=sheet_name, work_dir=work_dir)
        return replace(
            result,
            snapshots=tuple(replace(snapshot, version=self.current_version) for snapshot in result.snapshots),
        )
...
class GroupRenderer:
    def __init__(self, outcomes: dict[str, object]) -> None:
        ...
        self.snapshot_versions: list[tuple[str, str]] = []
    ...
    def render_group(self, group, snapshot, *, group_workbook, work_dir):
        ...
        self.snapshot_versions.append((group.template_id, snapshot.version))
...
def _service(
    ...
    return MultiTemplateRenderService(
        preflight_runner=preflight_runner or BatchPreflight(),
        jobs=store,
...
def test_retry_failed_renders_only_the_failed_template_with_a_new_canary(tmp_path):
    ...
    assert "output_history" not in recovered["multi_template"]
...
@pytest.mark.parametrize(
    ...
    ((False, "v1"), (True, "v2"), (True, "v0")),
    ids=("template-disabled", "new-version-published", "version-rolled-back"),
)
def test_resume_keeps_task_creation_snapshot_across_publication_changes(tmp_path, active, current_version):
    source = tmp_path / "orders.xlsx"
    _write_orders(source, ("A", "B", "C"))
    preflight = PublicationStatePreflight()
    service, _, renderer = _service(tmp_path, {"B": "system"}, preflight_runner=preflight)
    parent = service.preflight({"order_file": str(source), "sheet_name": "订单"})
    interrupted = service.execute(parent["job_id"])
    preflight.active = active
    preflight.current_version = current_version
    renderer.outcomes["B"] = "completed"
    ...
    assert [item["template_version"] for item in resumed["multi_template"]["template_checkpoints"]] == ["v1", "v1", "v1"]
    assert renderer.snapshot_versions == [("A", "v1"), ("B", "v1"), ("B", "v1"), ("C", "v1")]
    assert Path(resumed["outputs"]["primary_output"]).is_file()
