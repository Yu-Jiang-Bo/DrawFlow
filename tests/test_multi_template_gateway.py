...
def test_multi_template_parent_job_never_exposes_or_downloads_partial_output(tmp_path):
    ...
    try:
        ...
        with pytest.raises(urllib.error.HTTPError) as api_error:
            urllib.request.urlopen(f"{base_url}/api/jobs/{record['job_id']}/download/partial_output")
        with pytest.raises(urllib.error.HTTPError) as local_error:
            urllib.request.urlopen(f"{base_url}/local/jobs/{record['job_id']}/output/partial")
    finally:
        _stop(server, worker)
    ...
    assert "download_partial_output" not in detail["actions"]
    assert api_error.value.code == 404
    assert local_error.value.code == 404
