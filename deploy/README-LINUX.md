...
编辑 `drawflow.env`，只在中央服务上填写 `DRAWFLOW_LLM_API_KEY`、`DRAWFLOW_LLM_BASE_URL` 和模型。V2 真实样例预览使用独立的 `DRAWFLOW_PREVIEW_WORKER_SECRET`；自动识别 OpenType/PUA 尾巴字形的 Illustrator 扫描使用独立的 `DRAWFLOW_SCAN_WORKER_SECRET`。两者都至少生成 32 字节随机值，且只通过受控环境配置交给对应的可信 Windows 工作端，普通浏览器不得获取。不要复用两个密钥，也不要将真实密钥写入仓库、日志或发布包。
